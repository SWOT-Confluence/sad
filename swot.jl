...
"""
function write_output(reachid, valid, outdir, A0, n, Qa, Qu, W, Wr)
...
function main()
    ...
    A0 = missing
    n = missing
    Qa = Array{Missing}(missing, 1, size(Wr,1))
    Qu = Array{Missing}(missing, 1, size(Wr,1))
    if all(ismissing, H) || all(ismissing, W) || all(ismissing, S)
        println("$(reachid): INVALID")
        write_output(reachid, 0, outdir, A0, n, Qa, Qu, W, Wr)
    else
        ...
        if ismissing(Qp)
            ...
            try
                ...
                write_output(reachid, 1, outdir, A0, n, Qa, Qu, W, Wr)
            catch
                println("$(reachid): INVALID")
                write_output(reachid, 0, outdir, A0, n, Qa, Qu, W, Wr)
            end
